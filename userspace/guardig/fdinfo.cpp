...
///////////////////////////////////////////////////////////////////////////////
// guardig_fdinfo inomlementation
///////////////////////////////////////////////////////////////////////////////
template<> guardig_fdinfo_t::guardig_fdinfo()
{
...
template<> void guardig_fdinfo_t::reset()
{
	m_type = SCAP_FD_UNINITIALIZED;
	m_flags = FLAGS_NONE;
	m_callbaks = NULL;
...
template<> string* guardig_fdinfo_t::tostring()
{
	return &m_name;
}

template<> char guardig_fdinfo_t::get_typechar()
{
	switch(m_type)
	{
	case SCAP_FD_FILE:
...
template<> char* guardig_fdinfo_t::get_typestring()
{
	switch(m_type)
	{
	case SCAP_FD_FILE:
...
template<> string guardig_fdinfo_t::tostring_clean()
{
...
template<> void guardig_fdinfo_t::add_filename(const char* fullpath)
{
	m_name = fullpath;
}

/*
template<> bool guardig_fdinfo_t::set_net_role_by_guessing(sinsp* inspector,
...
wildass_guess:
	if(!(pfdinfo->m_flags & (guardig_fdinfo_t::FLAGS_ROLE_CLIENT | guardig_fdinfo_t::FLAGS_ROLE_SERVER)))
	{
		//
...
template<> scap_l4_proto guardig_fdinfo_t::get_l4proto()
{
...
template<> void guardig_fdinfo_t::register_event_callback(guardig_pd_callback_type etype, guardig_protodecoder* dec)
{
...
template<> void guardig_fdinfo_t::unregister_event_callback(guardig_pd_callback_type etype, guardig_protodecoder* dec)
{
...
///////////////////////////////////////////////////////////////////////////////
// guardig_fdtable implementation
///////////////////////////////////////////////////////////////////////////////
guardig_fdtable::guardig_fdtable(guardig* inspector)
{
	m_inspector = inspector;
	reset_cache();
}

guardig_fdinfo_t* guardig_fdtable::add(int64_t fd, guardig_fdinfo_t* fdinfo)
{
	//
...
#endif
			pair<unordered_map<int64_t, guardig_fdinfo_t>::iterator, bool> insert_res = m_table.emplace(fd, *fdinfo);
	...
	else
	{
		...
		//
		if(it->second.m_flags & guardig_fdinfo_t::FLAGS_CLOSE_IN_PROGRESS)
		{
			//
			...
			// destroy it.
			//
			fdinfo->m_flags &= ~guardig_fdinfo_t::FLAGS_CLOSE_IN_PROGRESS;
			fdinfo->m_flags |= guardig_fdinfo_t::FLAGS_CLOSE_CANCELED;
...
void guardig_fdtable::erase(int64_t fd)
{
	unordered_map<int64_t, guardig_fdinfo_t>::iterator fdit = m_table.find(fd);
...
void guardig_fdtable::clear()
{
	m_table.clear();
}

size_t guardig_fdtable::size()
{
	return m_table.size();
}

void guardig_fdtable::reset_cache()
{
	m_last_accessed_fd = -1;
}
