...
#define TRACE_DEBUG_KERNEL(...)     { }
#define TRACE_INFO_KERNEL(...)      { }
#define TRACE_WARNING_KERNEL(...)   { }
#define TRACE_ERROR_KERNEL(...)     { }
#define TRACE_FATAL_KERNEL(...)     { while(1); }
...
#define GET_PROC_NAME() 									\
	char proc_name[sizeof(current->comm)]; 					\
	task_lock(current); 									\
	memcpy(proc_name, current->comm, sizeof(proc_name)); 	\
	proc_name[sizeof(proc_name) - 1] = '\0'; 				\
	task_unlock(current);

// Trace compilation depends on TRACE_LEVEL value
#if (TRACE_LEVEL >= TRACE_LEVEL_DEBUG)
#define TRACE_DEBUG(...)      		{ printf ("-D- [%s]: ", __PRETTY_FUNCTION__); printf( __VA_ARGS__); printf ("\n\r"); }
#define TRACE_DEBUG_WP(...)   		{ printf(__VA_ARGS__); }
#define TRACE_DEBUG_KERNEL(...) 	{ GET_PROC_NAME(); printk("-D- [%s] %s: ", proc_name, __PRETTY_FUNCTION__); printk( __VA_ARGS__); printk ("\n\r"); }
#else
#define TRACE_DEBUG(...)      { }
#define TRACE_DEBUG_WP(...)   { }
#endif
...
#define TRACE_INFO(...)       	{ printf ("-I- [%s]: ", __PRETTY_FUNCTION__); printf(__VA_ARGS__); printf ("\n\r");}
#define TRACE_INFO_WP(...)    	{ printf(__VA_ARGS__); }
#define TRACE_INFO_KERNEL(...) 	{ GET_PROC_NAME(); printk("-I- [%s] %s: ", proc_name, __PRETTY_FUNCTION__); printk( __VA_ARGS__); printk ("\n\r"); }
#else
#define TRACE_INFO(...)       { }
#define TRACE_INFO_WP(...)    { }
#endif
...
#define TRACE_WARNING(...)    		{ printf ("-W- [%s]", __PRETTY_FUNCTION__); printf(__VA_ARGS__); printf ("\n\r");}
#define TRACE_WARNING_WP(...) 		{ printf(__VA_ARGS__); }
#define TRACE_WARNING_KERNEL(...) 	{ GET_PROC_NAME(); printk("-W- [%s] %s: ", proc_name, __PRETTY_FUNCTION__); printk( __VA_ARGS__); printk ("\n\r"); }
#else
#define TRACE_WARNING(...)    { }
#define TRACE_WARNING_WP(...) { }
#endif
...
#define TRACE_ERROR(...)     		{ printf ("-E- [%s]", __PRETTY_FUNCTION__); printf(__VA_ARGS__); printf ("\n\r"); }
#define TRACE_ERROR_WP(...)   		{ printf(__VA_ARGS__); }
#define TRACE_ERROR_KERNEL(...) 	{ GET_PROC_NAME(); printk("-E- [%s] %s: ", proc_name, __PRETTY_FUNCTION__); printk( __VA_ARGS__); printk ("\n\r"); }
#else
#define TRACE_ERROR(...)      { }
#define TRACE_ERROR_WP(...)   { }
#endif
...
#define TRACE_FATAL_KERNEL(...) 	{ GET_PROC_NAME(); printk("-F- [%s] %s: ", proc_name, __PRETTY_FUNCTION__); printk( __VA_ARGS__); printk ("\n\r"); }
#else
#define TRACE_FATAL(...)      { while(1); }
#define TRACE_FATAL_WP(...)   { while(1); }
#endif
