...
void sinsp_network_interfaces::update_tuple(ipv4tuple *pipv4info)
{
	...
	if(0 == pipv4info->m_sip)
	{
		uint32_t newaddr;
		newaddr = infer_ipv4_address(pipv4info->m_dip);

		if(newaddr == pipv4info->m_dip)
		{
			if(pipv4info->m_sport == pipv4info->m_dport)
			{
				return;
			}
		}

		pipv4info->m_sip = newaddr;
	}
	else
	{
		uint32_t newaddr;
		newaddr = infer_ipv4_address(pipv4info->m_sip);

		if(newaddr == pipv4info->m_sip)
		{
			if(pipv4info->m_sport == pipv4info->m_dport)
			{
				return;
